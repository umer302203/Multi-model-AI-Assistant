...
```mermaid
graph TD
    A["User Interface (Glassmorphism UI)"] -->|POST /generate| B["Flask Server (app.py)"]
    subgraph Backend Logic
        B --> C{"Model Router"}
        C -->|Llama| D["Llama Prompt Template"]
        C -->|Granite| E["Granite Prompt Template"]
        C -->|Mistral| F["Mistral Prompt Template"]
        D & E & F --> G["ChatWatsonx (LangChain Wrapper)"]
    end
    G -->|API Call| H["IBM watsonx.ai Foundation Models"]
    H -->|Raw Completion| I["JsonOutputParser (Pydantic Validation)"]
    I -->|Structured JSON| B
    B -->|Response JSON| A
```
